...
description: Writing documents for Claude. Use when creating or editing skills, or modifying CLAUDE.md.
---

Reference for writing any document Claude consumes: a skill, a `CLAUDE.md`, a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable, since the agent takes the same _process_ every run rather than producing the same output.
