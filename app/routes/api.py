...
def create_routine():
    ...
    # Pre-fetch existing exercises to avoid N+1 queries
    requested_ex_names = [
        (ex_data.get("name") or "").strip().lower()
        for ex_data in exercises_data
        if (ex_data.get("name") or "").strip()
    ]

    existing_exercises = {}
    if requested_ex_names:
        exercises = Exercise.query.filter(db.func.lower(Exercise.name).in_(requested_ex_names)).all()
        existing_exercises = {ex.name.lower(): ex for ex in exercises}
